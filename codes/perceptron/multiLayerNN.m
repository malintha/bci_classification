...
iterations=3514;%iterations=3514;

parameter=multilayerNeuralNetwork(X_input,Y_label,learningRate,iterations,dataTest,getOnehotVectorMode(Yte));
...
W1 = normrnd(0,1,middlelayerSize, m+1)*.0001;
%50 weights for unput to 2nd layer, 51th weight for bias
W2 = normrnd(0,1,p, middlelayerSize+1)*.0001; 
maxTestAcc=0;
recorderedIter=0;
for i=1:iterations
...
fprintf('Iteration : %d, max acc : %f \n',recorderedIter, maxTestAcc*1.11);  
end
function acc = accuracyCalcu(Y,yPred)
[m,n]=size(Y);
count=0;
for i=1:n
    count=count+sum((Y(:,i)&yPred(:,i)));
end
acc=(count*1.11)/n*100;
end
